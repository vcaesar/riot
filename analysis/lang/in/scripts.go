...
var decompositions = [][]rune{
	...
	{0x05, 0x3E, -1, 0x06, flag(unicode.Devanagari) | flag(unicode.Bengali) | flag(unicode.Gurmukhi) |
		flag(unicode.Gujarati) | flag(unicode.Oriya)},
...
func compose(ch0 rune, script0 *unicode.RangeTable, scriptData *ScriptData, input []rune, pos,
	inputLen int) []rune {
	if pos+1 >= inputLen {
